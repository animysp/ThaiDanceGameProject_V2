    ...
    public class VideoCapture : VideoCaptureBase
    {
        ...
        public override void StartCapture()
        {
            ...
            if (mode == ModeType.LOCAL)
            {
                ...
                {
                    int counter = 0;
                    while (GameManager.instance.VideoNumsUsed.Contains(counter))
                    {
                        counter++;
                    }
                    GameManager.instance.VideoNumsUsed.Add(counter);
                    filePath = Application.dataPath + "/StreamingAssets/videoclip" + counter + ".mp4";
                }
                /*
                if(this.gameObject.name == "CharacterKinectControllerCam")
                {
                    filePath = PathConfig.SaveFolder + StringUtils.GetMp4FileNameForKinect();
                }
                else
                {
                    filePath = PathConfig.SaveFolder + StringUtils.GetMp4FileNameForMocap();
                }
                filePath = PathConfig.SaveFolder + StringUtils.GetMp4FileName(StringUtils.GetRandomString(5));
                */
            }
